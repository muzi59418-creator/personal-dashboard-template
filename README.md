...
一个面向个人工作的本地化管理仪表板：无需后端和数据库，下载后即可运行，也可以通过 GitHub Template 创建自己的版本并继续二次开发。

当前版本：**v1.4.4** · [查看更新日志](CHANGELOG.md)

## 项目特点

- 数据默认保存在当前浏览器，不依赖服务器和数据库。
- 不内置账号体系，安装依赖后即可使用。
...
- 当前版本不需要后端服务、数据库或账号体系。
- 当前没有真实云同步和多人协作能力。
- 换浏览器、换设备或清理浏览器数据后，数据不会自动同步。
- 仓库仅包含虚构的通用演示数据，不包含个人工作记录、客户信息、联系方式、账号、密钥、token、本地私人路径或内部业务名称。
...
本轮为 v1.4.4 的项目介绍和展示素材补充优化，没有修改应用版本、数据结构、统计口径、`localStorage` key 或 `SCHEMA_VERSION`。
