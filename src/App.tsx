...
function DashboardApp({ accountName, onSignOut, cloudProbe, onCloudInitialized }: DashboardAppProps) {
  ...
  const actions = useMemo(
    () => ({
      ...
      createIdeaAction: (input: IdeaInput) => runAction(() => createIdea(input), "灵感已保存"),
      updateIdeaAction: (id: string, input: IdeaInput) => runAction(() => updateIdea(id, input), "灵感已更新"),
      deleteIdeaAction: (id: string) => runAction(() => deleteIdea(id), "灵感已删除"),
