...
assert.equal(migrated.appVersion, "1.2.0", "旧数据迁移补 appVersion");
assert.equal(migrated.schemaVersion, "1.2.0", "旧数据迁移补 schemaVersion");
...
assert.equal(JSON.parse(exported).appVersion, "1.2.0", "新 JSON 导出包含 appVersion");
assert.equal(JSON.parse(exported).schemaVersion, "1.2.0", "新 JSON 导出包含 schemaVersion");
